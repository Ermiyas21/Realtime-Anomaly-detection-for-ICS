...
# Define latency and throughput variables
total_batches = 0
total_data_points = 0
...
def predict_and_detect(batch_df):
    ...
    # Increment the total batches processed
    ...
    # Measure latency
    batch_latency = batch_processing_time 
    #total_latency += processing_time  

    # Increment total data points processed
    
    #total_data_points += result_df.count() 
    ...
    avg_latency = batch_latency / total_batches   

    # Print latency and throughput
    print(f"Latency: {total_processing_time } seconds")  
    print(f"Average Latency: {avg_latency} seconds") 
    print(f"Throughput: {throughput:.2f} data points per second") 

    #print(f"Latency for batch {total_batches}: {batch_latency:.4f} seconds")
    #print(f"Throughput: {throughput:.2f} data points per second")  
     # Write the calculated values to InfluxDB 
    # Write throughput and latency to InfluxDB
    client = InfluxDBClient(url=influxdb_url, token=token, org=org)
    write_api = client.write_api(write_options=WriteOptions(batch_size=1, flush_interval=1, jitter_interval=1))
    latency_point = Point("latency").field("value", str(total_processing_time)).time(time.time_ns()) 
    throughput_point = Point("throughput").field("value", str(throughput)).time(time.time_ns())
    write_api.write(bucket=bucket_, org=org, record=latency_point) 
    write_api.write(bucket=bucket_, org=org, record=throughput_point)    

    latency_point = Point("latency").field("value", str(avg_latency)).time(time.time_ns()) 
    throughput_point = Point("throughput").field("value", str(throughput)).time(time.time_ns())
    write_api.write(bucket=bucket_, org=org, record=latency_point)
    write_api.write(bucket=bucket_, org=org, record=throughput_point)  
...
def write_to_influxdb(df, total_processing_time, throughput, bucket_, token):
    ...
    # Write latency and throughput to InfluxDB
    data_points = [
        Point(measurement)
        .field('latency', total_processing_time)
        .field('throughput', throughput) 
    ]
    
    # Convert DataFrame to InfluxDB Point format  
    for row in df.collect():
        data_points.append(Point(measurement).field(col, value) for col, value in row.asDict().items())
...
#v spark-submit --packages org.apache.spark:spark-sql-kafka-0-10_2.12:3.3.0 /sparkScripts/streamingProcess2.py
